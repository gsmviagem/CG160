...
const MAX_MS = 3 * 60 * 1000; // 3 min timeout
const POLL_MS = 5_000;
...
// Call this from GenerateButton to activate the banner
export function setGenerating(type: 'ideas' | 'script') {
...
export function GeneratingBanner() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      setElapsed(secs);
      router.refresh();
      if (Date.now() - state.startedAt > MAX_MS) {
        clearGenerating();
      }
    }, POLL_MS);
    ...
    <div className="flex items-center gap-3 bg-indigo-950 border border-indigo-700 rounded-lg px-4 py-3 mb-5">
      ...
      <div className="flex-1 min-w-0">
        ...
        <span className="text-xs text-indigo-400 ml-2">
          {elapsed < 10
            ? 'iniciando Inngest job'
            : elapsed < 30
            ? 'modelo gerando conteúdo'
            : elapsed < 60
            ? 'quase pronto'
            : `${elapsed}s — pode levar até 2min`}
        </span>
      </div>
